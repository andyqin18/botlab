 ...
 *  - Add a rotation element to the StraightManeuverController
    ...
    */
    mbot_lcm_msgs::twist2D_t updateCommand(void)
    {
        ...
        if(!targets_.empty() && !odomTrace_.empty())
        {
            ...
            // TODO: we could not drive in a square when the state was set to smart, so we decided to
            // make straight-line motions and turns disjoint actions.
            // if (state_ == SMART)
            // {
...
int main(int argc, char** argv)
{
    ...
    constexpr float MAX_LINEAR_VEL = 0.2;  // m/s
    constexpr float MAX_ANGULAR_VEL = M_PI_2;  // rad/s
    ...
    while(true)
    {
    	...
    	if(controller.timesync_initialized()){
            ...
            // Fwd vel
            if (cmd.vx > MAX_LINEAR_VEL) cmd.vx = MAX_LINEAR_VEL;
            else if (cmd.vx < -MAX_LINEAR_VEL) cmd.vx = -MAX_LINEAR_VEL;

            // Angular vel
            if (cmd.wz > MAX_ANGULAR_VEL) cmd.wz = MAX_ANGULAR_VEL;
            else if (cmd.wz < -MAX_ANGULAR_VEL) cmd.wz = -MAX_ANGULAR_VEL;
