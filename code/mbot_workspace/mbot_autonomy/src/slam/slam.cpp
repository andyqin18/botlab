...
void OccupancyGridSLAM::handleLaser(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const mbot_lcm_msgs::lidar_t* scan)
{
    ...
    // std::cerr << "[DEBUG] mode: " << mode_ << " (should not be equal to " << mapping_only <<")\n";
    // std::cerr << "[DEBUG] haveOdom: " << haveOdom << '\n';
    // std::cerr << "[DEBUG] odometryPoses size: " << odometryPoses_.size() << '\n';
    // std::cerr << "[DEBUG]: scan times size " << scan->times.size() << "\n";
    // std::cerr << "[DEBUG] odom, scan timestamps " << odometryPoses_.front().utime << " / " << scan->times.front() << '\n';
...
{
    // std::cerr << "[TRACE] OccupancyGridSLAM::updateLocalization\n";
    // std::cerr << "[DEBUG] (haveMap_): " << haveMap_ << " (mode:) " << mode_ << "\n";
    if(haveMap_ && (mode_ != mapping_only))
    {
        auto time_1 = time(NULL);
        previousPose_ = currentPose_;
        ...
        auto time_2 = time(NULL);
        std::cerr << "[INFO] time for partile filters" << time_2 - time_1 << std::endl;
        auto particles = filter_.particles();

        lcm_.publish(SLAM_POSE_CHANNEL, &currentPose_);
        std::cerr << "ATTEMPTING TO PUBLISH PARTICLES; count = " << particles.num_particles << "\n";
