                                           ...
                                           double minFrontierLength)
{
    ...
    */
    ...
    std::cerr << "[DEBUG] robotpose: " << robotPose.x << ", " << robotPose.y << "\n";
    std::cerr << "[DEBUG] robotcell: " << robotCell.x << ", " << robotCell.y << "\n";
    ...
    std::cerr << "[DEBUG]: cellqueue empty: " << cellQueue.empty() << std::endl;
    while(!cellQueue.empty())
    {
        Point<int> nextCell = cellQueue.front();
        cellQueue.pop();

        if (map.logOdds(nextCell.x, nextCell.y) >= 0) {
            std::cerr << "[PANIC] BFS cell is not a free cell; value=" << (int) map.logOdds(nextCell.x, nextCell.y) << "\n";
        }
        ...
        for(int n = 0; n < kNumNeighbors; ++n)
        {
            ...
            // std::cerr << "[DEBUG] neighbor log odds: " << (int) map(neighbor.x, neighbor.y) << "\n";

            // If the cell has been visited or isn't in the map, then skip it
            // std::cerr << "[DEBUG]: in grid: " << map.isCellInGrid(neighbor.x, neighbor.y) << std::endl;
            // std::cerr << "[DEBUG]: find neighbor: " << (visitedCells.find(neighbor) != visitedCells.end()) << std::endl;
            ...
            {
                std::cerr << "[DEBUG] found frontier neighbor\n";
                ...
                std::cerr << "[DEBUG]: cell.size * meterspercell" << f.cells.size() * map.metersPerCell() << std::endl;
                                            ...
                                            const MotionPlanner& planner)
{
    ...
    */
    ...
    std::cerr << __PRETTY_FUNCTION__ << ":" << __LINE__ << "\n\t" << "Path start: " << path.path.front().x << ", " << path.path.front().y << "\n\t" << "Path end: " << path.path.back().x << ", " << path.path.back().y << "\n";
...
bool is_frontier_cell(int x, int y, const OccupancyGrid& map)
{
    ...
    {
        // std::cerr << "[DEBUG]: is_frontier_cell BAD CASE\n";
        return false;
    }
