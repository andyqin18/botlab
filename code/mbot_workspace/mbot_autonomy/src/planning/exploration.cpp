...
void Exploration::handleMap(const lcm::ReceiveBuffer* rbuf, const std::string& channel, const mbot_lcm_msgs::occupancy_grid_t* map)
{
    ...
    haveNewMap_ = true;
    std::cerr << "[DEBUG] Received new map\n";
}
...
{
    std::lock_guard<std::mutex> autoLock(dataLock_);
    // std::cerr << "[DEBUG]: have new map: " << haveNewMap_ << "[DEBUG]: have new pose: " << haveNewPose_ << std::endl;
...
void Exploration::copyDataForUpdate(void)
{
    ...
    std::cerr << "[DEBUG] Set robot pose to " << currentPose_.x << ", " << currentPose_.y << "\n";
...
int8_t Exploration::executeExploringMap(bool initialize)
{
    ...
    */
    ...
    std::vector<frontier_t> frontiers = find_map_frontiers(currentMap_, currentPose_, minFrontierLength);
    
    std::cerr << "[DEBUG] number of frontiers: " << frontiers.size() << "\n";

    frontier_processing_t frontier_processing = plan_path_to_frontier(frontiers, currentPose_, currentMap_, planner_);
    ...
    std::cerr << "[DEBUG] pathlen: " << frontier_processing.path_selected.path_length << std::endl;
    if (frontier_processing.path_selected.path_length == 1) {
        status.status = mbot_lcm_msgs::exploration_status_t::STATUS_COMPLETE; 
    } else {
        status.status = mbot_lcm_msgs::exploration_status_t::STATUS_IN_PROGRESS;
        currentPath_ = frontier_processing.path_selected;
        // lcmInstance_->publish(CONTROLLER_PATH_CHANNEL, &currentPath_);
        std::cerr << "[DEBUG]: path updated \n";
    }
